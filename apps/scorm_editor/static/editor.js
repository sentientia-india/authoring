(() => {
  ...
  // frontend/src/move-between-lists.js
  function moveBetweenLists(sourceList, sourceIndex, destList, destIndex) {
    if (!Array.isArray(sourceList) || !Array.isArray(destList)) return void 0;
    if (sourceIndex < 0 || sourceIndex >= sourceList.length) return void 0;
  ...
  // frontend/src/rich-text.js
  var ALLOWED_TAGS = /* @__PURE__ */ new Set(["strong", "em", "u", "a", "ul", "ol", "li"]);
  var BLOCK_LIKE_END_TAGS = /* @__PURE__ */ new Set(["li", "ul", "ol", "p", "div"]);
  ...
  function parseAttrs(attrString) {
    ...
    while (match = ATTR_RE.exec(attrString || "")) {
      ...
      var value = match[3] !== void 0 ? match[3] : match[4] !== void 0 ? match[4] : match[2] || "";
      attrs[name] = value;
    }
    return attrs;
  }
  ...
  var SUPPRESS_CONTENT_TAGS = /* @__PURE__ */ new Set(["script", "style"]);
...
3. "-" cannot repeat`);
  ...
  var index_default8 = BulletList;

  // node_modules/@tiptap/extension-ordered-list/dist/index.js
  var index_default9 = OrderedList;

  // node_modules/@tiptap/extension-list-item/dist/index.js
  var index_default10 = ListItem;
  ...
  var RICH_TEXT_EXTENSIONS = [
    ...
    index_default5,
    index_default6,
    index_default7.configure({ openOnClick: false, autolink: false, protocols: ["http", "https", "mailto"] }),
    index_default8,
    index_default9,
    index_default10
  ];
  (function() {
    ...
    var editorToken = function() {
      ...
      if (fromUrl) {
        try {
          sessionStorage.setItem("course-studio-token", fromUrl);
        } catch (e) {
        }
        return fromUrl;
      }
      try {
        return sessionStorage.getItem("course-studio-token") || "";
      } catch (e) {
        return "";
      }
    }();
    ...
    window.fetch = function(input, init2) {
      ...
      if (typeof input === "string" && input.charAt(0) === "/" && editorToken) {
        init2 = Object.assign({}, init2);
        init2.headers = Object.assign({}, init2.headers, { Authorization: "Bearer " + editorToken });
      }
    ...
    function findLesson(lessonKey) {
      ...
      return module ? { module, lesson: module.lessons[Number(parts[1])], mi: Number(parts[0]), li: Number(parts[1]) } : null;
    }
    function findBlock(cbId) {
      var hit = null;
      lessonsOf(state.course).forEach(function(row) {
        (row.lesson.content_blocks || []).forEach(function(block) {
          if (block.id === cbId) hit = { block, row };
        });
      });
      return hit;
    }
    function findActivity(activityId) {
      ...
      lessonsOf(state.course).forEach(function(row) {
        (row.lesson.activities || []).forEach(function(activity) {
          if ((activity.activity_id || activity.id) === activityId) hit = { activity, row };
        });
      });
      (state.course.modules || []).forEach(function(module) {
        (module.activities || []).forEach(function(activity) {
          if ((activity.activity_id || activity.id) === activityId) hit = { activity, row: null };
        });
      });
      return hit;
    }
    function findQuestion(questionId) {
      ...
      (final.questions || []).forEach(function(question) {
        if (question.id === questionId) hit = { question, home: final.questions };
      });
      lessonsOf(state.course).forEach(function(row) {
        (row.lesson.quiz_questions || []).forEach(function(question) {
          if (question.id === questionId) hit = { question, home: row.lesson.quiz_questions };
        });
      });
      return hit;
    }
    ...
    function renderReview() {
      ...
      Promise.all([
        fetch("/api/revisions/" + state.session).then(function(res) {
          return res.json();
        }),
        fetch("/api/collaboration/" + state.session).then(function(res) {
          return res.json();
        }),
        fetch("/api/accessibility/" + state.session).then(function(res) {
          return res.json();
        }),
        fetch("/api/localization/" + state.session).then(function(res) {
          return res.json();
        }),
        fetch("/api/generation/" + state.session).then(function(res) {
          return res.json();
        })
      ]).then(function(rows) {
        ...
        heading.textContent = "Review & approval";
        box.appendChild(heading);
        var form = document.createElement("form");
        form.className = "review-form";
        var input = document.createElement("textarea");
        input.setAttribute("aria-label", "New review comment");
        ...
        form.addEventListener("submit", function(event) {
          event.preventDefault();
          postCollaboration("comment", { message: input.value, target: state.selected.kind || "course" }).catch(function(error) {
            toast(error.message);
          });
        });
        ...
        [["approved", "Approve revision"], ["changes_requested", "Request changes"]].forEach(function(choice) {
          var button = document.createElement("button");
          button.className = "ghost";
          button.textContent = choice[1];
          button.addEventListener("click", function() {
            postCollaboration("approval", { decision: choice[0] }).catch(function(error) {
              toast(error.message);
            });
          });
        ...
        var generationStatus = document.createElement("p");
        generationStatus.className = "review-row";
        generationStatus.setAttribute("role", "status");
        generationStatus.textContent = generation.status.replace("_", " ") + " / " + generation.progress + "% / " + (generation.modules || []).filter(function(item) {
          return item.status === "succeeded";
        }).length + " of " + (generation.modules || []).length + " modules";
        box.appendChild(generationStatus);
        var generationActions = document.createElement("div");
        generationActions.className = "review-actions";
        var generationButton = document.createElement("button");
        generationButton.className = "primary";
        generationButton.textContent = generation.status === "failed" || generation.status === "cancelled" ? "Retry incomplete modules" : "Generate approved outline";
        generationButton.disabled = generation.status === "queued" || generation.status === "running";
        generationButton.addEventListener("click", function() {
          fetch("/api/generation/" + state.session, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "start" }) }).then(function(res) {
            return res.json();
          }).then(function(data) {
            if (!data.ok) throw new Error(data.error || "Generation failed to start");
            renderReview();
          }).catch(function(error) {
            toast(error.message);
          });
        ...
        cancelButton.disabled = generation.status !== "queued" && generation.status !== "running";
        cancelButton.addEventListener("click", function() {
          fetch("/api/generation/" + state.session, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "cancel" }) }).then(function(res) {
            return res.json();
          }).then(function(data) {
        ...
        generationActions.appendChild(cancelButton);
        box.appendChild(generationActions);
        if (generation.status === "queued" || generation.status === "running") {
          setTimeout(renderReview, 1e3);
        }
        var revisionTitle = document.createElement("h4");
        revisionTitle.textContent = "Revision history";
        box.appendChild(revisionTitle);
        revisions.forEach(function(revision) {
          var row = document.createElement("p");
          row.className = "review-row";
          row.textContent = "Revision " + revision.version + " \xB7 " + revision.reason + " \xB7 " + revision.actor;
          box.appendChild(row);
        });
        var commentTitle = document.createElement("h4");
        commentTitle.textContent = "Comments";
        box.appendChild(commentTitle);
        collaboration.comments.forEach(function(comment) {
          var row = document.createElement("p");
          row.className = "review-row";
          row.textContent = (comment.resolved ? "Resolved \xB7 " : "") + comment.actor + ": " + comment.message;
          box.appendChild(row);
        });
        var accessibilityTitle = document.createElement("h4");
        accessibilityTitle.textContent = "Accessibility report";
        box.appendChild(accessibilityTitle);
        var accessibilitySummary = document.createElement("p");
        accessibilitySummary.className = "review-row";
        accessibilitySummary.textContent = (accessibility.status === "pass" ? "Pass" : "Export blocked") + " / " + accessibility.summary.blockers + " blockers / " + accessibility.summary.warnings + " warnings";
        box.appendChild(accessibilitySummary);
        accessibility.issues.forEach(function(item) {
          var row = document.createElement("p");
          row.className = "review-row";
          row.textContent = item.severity.toUpperCase() + " / " + item.path + " / " + item.message;
          box.appendChild(row);
        });
        var localizationTitle = document.createElement("h4");
        localizationTitle.textContent = "Localization";
        box.appendChild(localizationTitle);
        var localeForm = document.createElement("form");
        localeForm.className = "review-form";
        var localeInput = document.createElement("input");
        localeInput.required = true;
        localeInput.placeholder = "Locale, e.g. es-MX";
        localeInput.setAttribute("aria-label", "New locale");
        var localeButton = document.createElement("button");
        localeButton.className = "primary";
        localeButton.type = "submit";
        localeButton.textContent = "Add locale";
        localeForm.append(localeInput, localeButton);
        localeForm.addEventListener("submit", function(event) {
          event.preventDefault();
          fetch("/api/localization/" + state.session, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "add_locale", locale: localeInput.value }) }).then(function(res) {
            return res.json();
          }).then(function(data) {
            if (!data.ok) throw new Error(data.error || "Locale update failed");
            renderReview();
          }).catch(function(error) {
            toast(error.message);
          });
        });
        box.appendChild(localeForm);
        Object.keys(localization.locales).sort().forEach(function(locale) {
          var item = localization.locales[locale];
          var row = document.createElement("div");
          row.className = "review-row";
          var label = document.createElement("span");
          label.textContent = locale + (locale === localization.base_locale ? " / inherited source" : " / " + Object.keys(item.overrides || {}).length + " overrides");
          var status = document.createElement("select");
          ["source", "draft", "in_review", "approved"].forEach(function(value) {
          ...
          status.disabled = locale === localization.base_locale;
          status.setAttribute("aria-label", locale + " translation status");
          status.addEventListener("change", function() {
            fetch("/api/localization/" + state.session, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "set_status", locale, status: status.value }) }).then(function(res) {
              return res.json();
            }).then(function(data) {
              if (!data.ok) throw new Error(data.error || "Translation status failed");
              renderReview();
            }).catch(function(error) {
              toast(error.message);
            });
          ...
          if (locale !== localization.base_locale) {
            ...
            translationForm.className = "review-form";
            var pathInput = document.createElement("input");
            pathInput.placeholder = "Field path, e.g. course_title";
            pathInput.required = true;
            pathInput.setAttribute("aria-label", locale + " field path");
            var valueInput = document.createElement("textarea");
            valueInput.placeholder = "Translated value";
            valueInput.required = true;
            valueInput.setAttribute("aria-label", locale + " translated value");
            var translateButton = document.createElement("button");
            translateButton.type = "submit";
            translateButton.className = "ghost";
            translateButton.textContent = "Save translation";
            translationForm.append(pathInput, valueInput, translateButton);
            translationForm.addEventListener("submit", function(event) {
              event.preventDefault();
              fetch("/api/localization/" + state.session, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "set_override", locale, path: pathInput.value, value: valueInput.value }) }).then(function(res) {
                return res.json();
              }).then(function(data) {
                if (!data.ok) throw new Error(data.error || "Translation save failed");
                renderReview();
              }).catch(function(error) {
                toast(error.message);
              });
            });
            box.appendChild(translationForm);
          }
        });
      }).catch(function(error) {
        box.textContent = "Review data unavailable: " + error.message;
      });
    }
    function renderSources() {
      var box = $("tab-sources");
      if (!state.session || !box) return;
      fetch("/api/sources/" + state.session).then(function(res) {
        return res.json();
      }).then(function(data) {
        box.replaceChildren();
        var heading = document.createElement("h3");
        ...
        text.required = true;
        text.minLength = 20;
        text.placeholder = "Paste source text. It remains in the authoring workspace and is not added to the exported course.";
        text.setAttribute("aria-label", "Source text");
        var button = document.createElement("button");
        button.className = "primary";
        button.type = "submit";
        button.textContent = "Add source";
        form.append(title, text, button);
        form.addEventListener("submit", function(event) {
          event.preventDefault();
          fetch("/api/sources/" + state.session, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: title.value, text: text.value }) }).then(function(res) {
            return res.json();
          }).then(function(result) {
            if (!result.ok) throw new Error(result.error || "Source intake failed");
            toast("Source added. Use its ID in lesson citations.");
            renderSources();
          }).catch(function(error) {
            toast(error.message);
          });
    ...
    function exportZip() {
      fetch("/api/export/" + state.session, { method: "POST" }).then(function(res) {
    ...
    function createNewCourse(event) {
      ...
      fetch("/api/new", {
        ...
        setSaveStatus("Course creation failed");
        $("import-error").textContent = error.message;
      });
    }
    function select(sel) {
    ...
    function treeNode(options) {
      ...
      node.innerHTML = (options.draggable ? '<span class="grip" title="Drag to reorder (or focus + Ctrl+Arrow keys)">\u22EE\u22EE</span>' : "") + '<span class="kind">' + options.kind + '</span><span class="label">' + escapeHtml(options.label) + "</span>" + (options.onDelete ? '<button class="del" title="Delete">\u2715</button>' : "");
      ...
      if (options.draggable) {
        ...
        node.addEventListener("drop", function(event) {
          ...
          handleDrop2(payload, Object.assign({ position }, options.drag));
        });
    ...
    function handleDrop2(source, target) {
      if (!source || !target) return;
      if (source.type === "template") {
        insertTemplate(source.template, target);
        return;
      }
      var c = state.course;
    ...
    function renderTree() {
      ...
      (state.course.modules || []).forEach(function(module, mi) {
        tree.appendChild(treeNode({
          ...
          onSelect: function() {
            select({ kind: "module", mi });
          },
          onDelete: function() {
            state.course.modules.splice(mi, 1);
            save(true);
          }
        }));
        (module.lessons || []).forEach(function(lesson2, li) {
          var key = mi + ":" + li;
          tree.appendChild(treeNode({
            kind: "lesson",
            label: lesson2.title || "Lesson " + (li + 1),
            indent: 2,
            selected: sel.kind === "lesson" && sel.key === key,
            draggable: true,
            drag: { type: "lesson", mi, li },
            reorder: { scope: "lesson:" + mi, index: li, list: module.lessons },
            onSelect: function() {
              select({ kind: "lesson", key });
            },
            onDelete: function() {
      ...
      if (sel.kind === "lesson" && findLesson(sel.key)) {
        var found2 = findLesson(sel.key);
        var lesson = found2.lesson;
        ...
        (lesson.content_blocks || []).forEach(function(block, bi) {
          sub.appendChild(treeNode({
            ...
            draggable: true,
            drag: { type: "block", key: sel.key, bi },
            reorder: { scope: "block:" + sel.key, index: bi, list: lesson.content_blocks },
            onSelect: function() {
              select({ kind: "block", cbId: block.id, key: sel.key });
            },
            onDelete: function() {
              lesson.content_blocks.splice(bi, 1);
              save(true);
            }
          }));
        });
        (lesson.activities || []).forEach(function(activity, ai) {
          sub.appendChild(treeNode({
            ...
            draggable: true,
            drag: { type: "activity", key: sel.key, ai },
            reorder: { scope: "activity:" + sel.key, index: ai, list: lesson.activities },
            onSelect: function() {
              select({ kind: "activity", activityId: activity.activity_id || activity.id, key: sel.key });
            },
            onDelete: function() {
              lesson.activities.splice(ai, 1);
              save(true);
            }
          }));
        });
        (lesson.quiz_questions || []).forEach(function(question, qi) {
          sub.appendChild(treeNode({
            ...
            draggable: true,
            drag: { type: "question", home: sel.key, qi },
            reorder: { scope: "question:" + sel.key, index: qi, list: lesson.quiz_questions },
            onSelect: function() {
              select({ kind: "question", questionId: question.id });
            },
            onDelete: function() {
      ...
      if (sel.kind === "final") {
        ...
        finalQuestions.forEach(function(question, qi) {
          subF.appendChild(treeNode({
            ...
            selected: sel.kind === "question" && sel.questionId === question.id,
            draggable: true,
            drag: { type: "question", home: "final", qi },
            reorder: { scope: "question:final", index: qi, list: finalQuestions },
            onSelect: function() {
              select({ kind: "question", questionId: question.id });
            },
            onDelete: function() {
              finalQuestions.splice(qi, 1);
              save(true);
            }
          }));
        });
    ...
    function insertTemplate(templateId, target) {
      ...
      if (payload.target === "question" && state.selected.kind === "final") {
        ...
        if (!key) {
          toast("Select a lesson first (Structure tab), then insert.");
          return;
        }
        var found2 = findLesson(key);
        if (!found2) {
          toast("Select a lesson first.");
          return;
        }
        if (payload.target === "block") (found2.lesson.content_blocks = found2.lesson.content_blocks || []).push(payload.value);
        if (payload.target === "activity") (found2.lesson.activities = found2.lesson.activities || []).push(payload.value);
        if (payload.target === "question") (found2.lesson.quiz_questions = found2.lesson.quiz_questions || []).push(payload.value);
      }
      save(true);
    ...
    function renderPalette() {
      ...
      TEMPLATES.forEach(function(template) {
        ...
        card.draggable = true;
        card.innerHTML = '<span class="palette-icon">' + template.icon + "</span><div><strong>" + template.name + "</strong><span>" + template.note + '</span></div><button type="button">Insert</button>';
    ...
    function switchRow(labelText, value, onChange) {
      ...
      var wrap2 = document.createElement("label");
      wrap2.className = "switch";
      ...
      wrap2.appendChild(document.createElement("i"));
    ...
    function mediaEditor(owner) {
      ...
      wrap2.appendChild(field("Media type", selectInput(media ? media.kind : "none", ["none", "image", "video", "link"], function(value) {
        if (value === "none") {
          delete owner.media;
          save(true);
          return;
      ...
      if (media && media.kind) {
        wrap2.appendChild(field("URL (https\u2026 or assets/media/\u2026)", textInput(media.src || "", function(value) {
          set("src", value);
        }, "url")));
        wrap2.appendChild(field("Caption", textInput(media.caption || "", function(value) {
          set("caption", value);
        })));
        wrap2.appendChild(field("Alt text", textInput(media.alt || "", function(value) {
          set("alt", value);
        })));
        if (media.kind === "image") {
          if (media.src) {
    ...
    function questionEditor(question) {
      ...
      box.style.gap = "12px";
      box.appendChild(field("Question", textInput(question.question, function(value) {
      ...
      (question.options || []).forEach(function(option, index) {
        ...
        del2.style.border = "0";
        del2.style.color = "var(--muted)";
        del2.style.cursor = "pointer";
        del2.addEventListener("click", function() {
          question.options.splice(index, 1);
          question.correct_answers = (question.correct_answers || []).filter(function(a) {
            return question.options.indexOf(a) >= 0;
          });
          save(true);
        });
      ...
      box.appendChild(field("Explanation / feedback", textInput(question.explanation, function(value) {
    ...
    function activityEditor(activity) {
      ...
      box.appendChild(field("Title", textInput(activity.title, function(value) {
        activity.title = value;
        save(true);
      })));
      box.appendChild(field("Instructions / objective", textInput(activity.objective || activity.instructions, function(value) {
    ...
    function renderInspector() {
      ...
      if (sel.kind === "course") {
        ...
        box.appendChild(field("Title", textInput(course.course_title, function(value) {
          course.course_title = value;
          save(true);
        })));
        box.appendChild(field("Theme", selectInput(course.theme || "studio", ["studio", "compliance", "academy"], function(value) {
          course.theme = value;
          save(true);
        })));
        box.appendChild(sectionLabel("Brand kit"));
        var branding = course.branding = course.branding || {};
        box.appendChild(field("Organization", textInput(branding.organization || "", function(value) {
          branding.organization = value;
          save(true);
        })));
        box.appendChild(field("Logo URL", textInput(branding.logo_url || "", function(value) {
          branding.logo_url = value;
          save(true);
        })));
        box.appendChild(field("Primary color", textInput(branding.primary_color || "#1f6f5f", function(value) {
          branding.primary_color = value;
          save(true);
        })));
        box.appendChild(field("Typography", selectInput(branding.typography || "system", ["system", "outfit", "geist", "accessible"], function(value) {
          branding.typography = value;
          save(true);
        })));
        box.appendChild(field("Certificate footer", textInput(branding.certificate_footer || "", function(value) {
          branding.certificate_footer = value;
          save(true);
        })));
        box.appendChild(sectionLabel("Workflow"));
        var workflow = course.authoring_workflow = course.authoring_workflow || {};
        box.appendChild(switchRow("Outline approved", workflow.outline_approved === true, function(value) {
          workflow.outline_approved = value;
          save(false);
        }));
        ...
        box.appendChild(field("Timer seconds", textInput(options.timer_seconds || 20, function(value) {
      ...
      if (sel.kind === "module") {
        ...
        box.appendChild(field("Title", textInput(module.title, function(value) {
          module.title = value;
          save(true);
        })));
        box.appendChild(field("Duration (minutes)", textInput(module.duration_minutes, function(value) {
          module.duration_minutes = Number(value) || 10;
          save(true);
        }, "number")));
      ...
      if (sel.kind === "lesson") {
        ...
        box.appendChild(field("Title", textInput(lesson.title, function(value) {
          lesson.title = value;
          save(true);
        })));
        box.appendChild(field("Objective", textInput(lesson.objective, function(value) {
          lesson.objective = value;
          save(true);
        }, "area")));
        box.appendChild(field("Duration (minutes)", textInput(lesson.duration_minutes, function(value) {
          lesson.duration_minutes = Number(value) || 8;
          save(true);
        }, "number")));
      ...
      if (sel.kind === "block") {
        ...
        if (!foundBlock) {
          box.innerHTML = '<p class="inspector-empty">Block not found.</p>';
          return;
        }
        title.textContent = "Content block";
        var block = foundBlock.block;
        box.appendChild(field("Type", selectInput(
        ...
        box.appendChild(field("Text", textInput(block.text, function(value) {
          block.text = value;
          save(true);
        }, "area")));
      ...
      if (sel.kind === "activity") {
        ...
        if (!foundActivity) {
          box.innerHTML = '<p class="inspector-empty">Activity not found.</p>';
          return;
        }
      ...
      if (sel.kind === "question") {
        ...
        if (!foundQuestion) {
          box.innerHTML = '<p class="inspector-empty">Question not found.</p>';
          return;
        }
      ...
      if (sel.kind === "final") {
        ...
        box.appendChild(field("Title", textInput(final.title, function(value) {
          final.title = value;
          save(true);
        })));
        box.appendChild(field("Passing score (%)", textInput(final.passing_score, function(value) {
          final.passing_score = Number(value) || 80;
          save(true);
        }, "number")));
    ...
    var HIGHLIGHT_CSS = "[data-cb-id], [data-activity-id], [data-question-id], [data-lesson-id] { transition: outline-color .15s ease; outline: 2px solid transparent; outline-offset: 3px; }.studio-hover { outline-color: rgba(56,189,248,.8) !important; cursor: pointer; }.studio-selected { outline-color: rgba(45,212,191,.95) !important; }.studio-editing { outline-color: #f59e0b !important; background: rgba(245,158,11,.06); }";
    function canvasDoc() {
      try {
    ...
    function startInlineEdit(target) {
      ...
      var initialHtml = found2.block.text_html || "<p>" + escapeHtml(found2.block.text || "") + "</p>";
      ...
      var finish = function() {
        ...
        destroyActiveRichEditor();
        var changed = plainText && (plainText !== found2.block.text || sanitizedHtml !== (found2.block.text_html || ""));
        if (changed) {
          found2.block.text_html = sanitizedHtml;
          found2.block.text = plainText;
      ...
      doc3.addEventListener("mousedown", onDocMouseDown, true);
      activeRichEditor = { editor, overlay, target, onDocMouseDown, canvasDoc: doc3 };
    }
    function bindCanvas() {
      var doc3 = canvasDoc();
      if (!doc3 || !doc3.body) return;
      if (doc3.getElementById("studio-css")) return;
      var style2 = doc3.createElement("style");
      style2.id = "studio-css";
      style2.textContent = HIGHLIGHT_CSS;
      doc3.head.appendChild(style2);
      var hovered = null;
      doc3.addEventListener("mousemove", function(event) {
      ...
      doc3.addEventListener("click", function(event) {
        var target = editableTargetOf(event.target);
        if (!target) return;
        doc3.querySelectorAll(".studio-selected").forEach(function(el) {
          el.classList.remove("studio-selected");
        });
        target.classList.add("studio-selected");
        selectFromCanvas(target);
      }, true);
      doc3.addEventListener("dblclick", function(event) {
        var target = editableTargetOf(event.target);
    ...
    canvas.addEventListener("load", function() {
      bindCanvas();
      var doc3 = canvasDoc();
      if (doc3 && doc3.body && window.MutationObserver) {
        new MutationObserver(function() {
          bindCanvas();
        }).observe(doc3.body, { childList: true, subtree: false });
      }
    });
    ...
    if (params.get("session")) {
      fetch("/api/course/" + params.get("session")).then(function(res) {
        return res.json();
      }).then(function(data) {
    ...
    window.addEventListener("online", function() {
      setSaveStatus(state.conflicted ? "Conflict \xB7 reload required" : "Online \xB7 ready to save");
    });
