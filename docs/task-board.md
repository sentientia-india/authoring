...
| T-028 | Done | Codex | Complete hosted event coverage and measurable analytics quality. | `migrations/0010_analytics_operations.sql`, `hosted_repository.py`, `analytics.py`, authenticated analytics routes, tests | Append-only observations distinguish accepted, duplicate, and rejected ingestion; late/missing-context quality and learner timelines are tenant-scoped; stored email/report recipients use authenticated encryption. |
| T-029 | Done | Codex | Operate scheduled reports and transactional delivery as durable workers. | `analytics_worker.py`, `outbox_worker.py`, object storage, Docker Compose, deployment workflows, tests | Reports are claimed without duplicate workers, generated to tenant object storage, downloaded through tenant authorization, and emailed through the retry/dead-letter outbox; production deploys both isolated workers with a stable externalized PII key. |
| T-030 | Done | Codex | Complete secure embeds and verified custom-domain delivery. | `hosted_learning.py`, `hosted_repository.py`, `server.py`, `Caddyfile`, tests | Embeds require HTTPS origin allowlists and consent, publish resize messages without third-party cookies, and enforce access grants; custom domains serve only verified published releases and Caddy may issue certificates only after the database authorization check. |
| T-031 | Done | Codex | Make commerce provisioning durable and replay-safe. | `billing.py`, `billing_repository.py`, `licensing.py`, `communication.py`, migrations, PostgreSQL tests | Subscription snapshots cannot regress from stale Stripe events; one-time purchases grant hosted access without issuing SaaS licenses; product licenses and quotas are transactional PostgreSQL state; recipient and template PII are encrypted. |
| T-032 | Done | Codex | Enforce production operations and capacity gates. | observability/status endpoints, deployment workflows, `capacity_matrix.py`, incident runbooks | Candidate releases must pass health, smoke, and bounded 1x/3x/10x capacity checks before atomic promotion; failed candidates restart the prior SHA; operators have top-ten recovery procedures. |
| T-033 | Done | Codex | Complete release compatibility and distribution documentation. | quickstart, `server.json`, tool contracts, deployment and release policy | Client connection, version support, deprecation windows, expand/migrate/contract changes, registry publication, and external verification gates are explicit and testable. |
| T-034 | Done | Codex | Close Course Studio launch-blocker evidence. | `apps/scorm_editor`, focused editor/round-trip suites, release evidence | The preserved redesign covers production workflows, failure/recovery, safe imports, responsive accessibility, conflict control, and import-edit-export-reimport fidelity. |
| T-035 | Done | Codex | Remediate the internal production security review. | billing authorization, hash-pinned locks, commit-pinned Actions, CODEOWNERS, gitleaks, tests | Price-to-tier mapping is server-authoritative, portal customers are tenant-owned, builds are reproducible, the locked graph is vulnerability-audited, and workflow supply-chain controls are enforced. |
